...
```{r}
#| label: fig-uk-gdp
#| fig-cap: "Figure: UK GDP per capita (constant 2015 US$), with 2016 referendum and 2021 Trade and Cooperation Agreement marked by dashed lines."
#| fig-align: center

# Download GDP per capita (constant 2015 USD)
uk_gdp <- WDI(
...
ggplot(uk_gdp, aes(x = year, y = NY.GDP.PCAP.KD)) +
  geom_line(color = "#003399", size = 1.1) +
...
```{r}
# Max tradeflow observation for any year
slice_max(gravity %>% filter(iso3_d == "GBR"), tradeflow_comtrade_d) # Max import
slice_max(gravity %>% filter(iso3_o == "GBR"), tradeflow_comtrade_o) # Max export

# Average UK export to EU and non-EU countries in 2010
uk_exports_2010 %>% 
  group_by(eu_d) %>% 
  summarise(
    mean = mean(tradeflow_comtrade_o, na.rm = TRUE),
    median = median(tradeflow_comtrade_o, na.rm = TRUE)
  )
```
...
tab1 <- uk_exports_2010 %>% 
  select(year, starts_with(c("iso3", "eu")), tradeflow_comtrade_o) %>% 
  arrange(desc(tradeflow_comtrade_o)) %>% 
  slice(1:10)

ggplot(tab1, aes(x = iso3_d, y = tradeflow_comtrade_o)) +
  geom_point() +
  labs(
    title = "Top 10 UK Export Destinations (2010)",
...
tab2 <- uk_exports_2020 %>% 
  select(year, starts_with(c("iso3", "eu")), tradeflow_comtrade_o) %>% 
  arrange(desc(tradeflow_comtrade_o)) %>% 
  slice(1:10)

ggplot(tab2, aes(x = iso3_d, y = tradeflow_comtrade_o)) +
  geom_point() +
  labs(
    title = "Top 10 UK Export Destinations (2020)",
    x = "Destination Country",
    y = "Export Value"
  )
```
...
```{r}
# Load helper package for side-by-side plots
library(patchwork)
...
### EU vs Non-EU tradeflows (2010 vs 2020)
...
### UK trade with selected countries (2010–2020)

```{r}
# Focus countries
partners <- c("GBR", "DEU", "FRA", "ITA", "ESP", "NLD", "BEL", "CHE", "CAN")

uk_exports %>% 
  filter(iso3_d %in% partners) %>% 
  group_by(year, iso3_d) %>% 
  summarise(total_trade = sum(tradeflow_comtrade_o, na.rm = TRUE)) %>% 
  ggplot(aes(x = year, y = total_trade, color = iso3_d)) +
  geom_line(linewidth = 1.2) +
  labs(
    title = "UK Exports to Selected Countries (2010–2020)",
    x = "Year",
    y = "Export Value",
    color = "Country"
  )
